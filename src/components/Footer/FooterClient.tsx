...
const FooterClient: React.FC<FooterClientProps> = ({ initialVisits }) => {
  const [visits, setVisits] = useState<number>(initialVisits);

  // Refresh the count on the client so we always show the latest number.
  useEffect(() => {
    fetch('/api/visits')
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    ...
    >
      <div
        className="max-w-[1600px] w-full h-full mx-auto
          px-[20px] md:px-[30px] 
          gap-12 md:gap-6
      ...
      >
        ...
        <div className="flex justify-end gap-[26px]">
          <Link href="https://www.linkedin.com/in/markaxelus/" target="_blank">
            LinkedIn
          </Link>
          <Link href="https://github.com/markaxelus" target="_blank">
            Github
          </Link>
          <Link href="https://www.instagram.com/mrkaxelus/" target="_blank">
            Instagram
          </Link>
        </div>
        {/* Middle container */}
